...
with st.expander("📌 Panduan Format Excel (Klik untuk lihat)"):
    st.markdown("""
    **Berikut adalah aturan format Excel yang harus diikuti agar dashboard dapat memproses data dengan benar:**

    ✅ **Kolom Wajib:**
    - `Nm Unit`, `Nopd`, `Nama Op`, `Status`, `TMT`
    - `Klasifikasi` (**khusus untuk pajak Hiburan**)

    ✅ **Kolom Pembayaran Pajak per Bulan:**
    - Gunakan format nama kolom: `2024-01-01`, `2024-02-01`, ..., `2024-12-01`
    - Format isi sel harus berupa **angka (number)**, bukan teks
    - Hindari kolom dengan nama seperti: `Total`, `Rata-rata`, `Average`, `Sum of`, dll.

    ❌ **Contoh Kolom Tidak Valid:**
    - `Sum of Masa Jan`
    - `Total Pembayaran`
    - `Rata-rata Januari`

    📄 **Gunakan file contoh berikut sebagai acuan.**
    """)


with open("CONTOH_FORMAT_DASHBOARD.xlsx", "rb") as f:
    st.download_button(
        label="📥 Download Contoh Format Excel",
        data=f,
        file_name="CONTOH_FORMAT_DASHBOARD.xlsx",
...
total_wp = df_output["Nama Op"].nunique()
rata_rata = df_output["Total Pembayaran"].mean()
total_pembayaran = df_output["Total Pembayaran"].sum()
col1, col2, col3 = st.columns(3)
col1.metric("Total WP", total_wp)
col2.metric("Rata-rata Pembayaran", f"Rp {rata_rata:,.0f}")
col3.metric("Total Pembayaran", f"Rp {total_pembayaran:,.0f}")

if "Klasifikasi" in df_output.columns:
    st.subheader("🏷️ Bar Chart Berdasarkan Klasifikasi")
    klasif_group = df_output.groupby("Klasifikasi")["Total Pembayaran"].sum().reset_index()
    fig_klasif = px.bar(
        klasif_group, 
        x="Klasifikasi", 
        y="Total Pembayaran", 
        title="Total Pembayaran per Klasifikasi",
        color="Klasifikasi", 
        color_discrete_sequence=px.colors.qualitative.Pastel
    )
    st.plotly_chart(fig_klasif, use_container_width=True)